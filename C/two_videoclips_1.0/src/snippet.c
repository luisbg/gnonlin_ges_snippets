...
GstElement *pipeline, *sink, *comp, *compconvert, *video1, *video2;
...
int main (int argc, char *argv[])
{
    ...
    compconvert = gst_element_factory_make ("autovideoconvert", "convert");
    ...
    /* create a gnurisources */
    video1 = gst_element_factory_make ("gnlurisource", "video1");
    gst_bin_add (GST_BIN (comp), video1);
    video2 = gst_element_factory_make ("gnlurisource", "video2");
    gst_bin_add (GST_BIN (comp), video2);

    /* set the gnlurisource properties */
    g_object_set (G_OBJECT (video1),
        "uri", argv[1],
        "start", 0 * GST_SECOND,
        "duration", 3 * GST_SECOND,
        "media-start", 10 * GST_SECOND,
        "media-duration", 4 * GST_SECOND,
        NULL);
    g_object_set (G_OBJECT (video2),
        "uri", argv[2],
        "start", 3 * GST_SECOND,
        "duration", 6 * GST_SECOND,
        "media-start", 0 * GST_SECOND,
        "media-duration", 3 * GST_SECOND,
        NULL);

    if (! pipeline || ! comp || ! compconvert || ! sink || ! video1 || ! video2)
    {
